...
definition valid_batch_msg :: "'e polynomial \<Rightarrow> 'a eval_witness \<Rightarrow> 'e eval_position set \<Rightarrow> bool" where 
  "valid_batch_msg r_x w_B B = (w_B \<in> carrier G\<^sub>p \<and> degree r_x < max_deg \<and> card B < max_deg)"
...
Note, the proof is structured analogy to the normal KZG's evaluation biding proof.\<close>
...
locale BatchEvalKZG_PCS_binding = BatchEvalKZG_PCS_correct
begin 

text \<open>We prove two evaluation binding properties: 
1. the classical evaluation binding game from PCS
2. the binding game from the original KZG10 paper
...
subsection \<open>PCS evaluation binding\<close>
