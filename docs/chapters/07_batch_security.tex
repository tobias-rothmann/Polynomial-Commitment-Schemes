...
\end{enumerate}
Firstly, we argue why we do not think that the hiding game for the batched version can be reduced to the hiding game of the normal KZG. We outline specifically the difficulties in transitioning from \textit{CreateWitness} to \textit{CreateWitnessBatch}.
...
Note, that the majority of the inputs for the adversaries are equal, they take firstly the public key $PK$ and secondly a commitment $C$. The only value they differ in is the witness tuples. Specifically in the normal hiding game, the witnesses for the adversary are computed as 
'$\text{map } (\text{CreateWitness}(PK,\phi,i))\ I$', while the batched version computes them as '$\text{CreateWitnessBatch}(PK,\phi,B)$' for its respective adversary.
...
As little reminder, the result from \textit{CreateWitness} is a tuple $(i, \phi(i), \omega_i)$, where $\omega_i$ is the wintess for the point $(i,\phi(i))$. The result from \textit{CreateWitnessBatch} is a tuple $(B, r(x), \omega_B)$, where B is a set of points, $r(i)=\phi(i)$ for all $i\in B$, and is the wintess $\omega_B$ is a witness for all points $(i,r(i))$ for $i\in B$.
...
One could ideate to somehow carve $\psi_i$ out of $\psi_B$, as $\psi_B=\mathbf{g}^{\frac{\phi(\alpha)-r(\alpha)}{\prod_{i \in B}^{}(\alpha-i)}}$ already includes the divions by $\alpha-i$. However, this would require knowledge of $\frac{\phi(x)-r(x)}{\prod_{i \in B\backslash\{i\}}^{}(\alpha-i)}$, which again is a field element that dependents on $\alpha$. Hence this approach is also impossible by DL assumption.
...
For this theorem, we aim to show the theorem following theorem, which is the one firstly outlined in this section: 
\begin{lstlisting}[language=isabelle]
    theorem batch_hiding: "batch_hiding_advantage $\mathcal{A}$ 
        $\le$ t_SDH.advantage (reduction $\mathcal{A}$)"
\end{lstlisting}
