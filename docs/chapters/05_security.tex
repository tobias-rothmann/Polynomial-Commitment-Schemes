...
The in the original paper \parencite{KZG} introduced reduction algorithm $\mathcal{B}$ cannot be trivially turned into a reduction adversary that works for a game-based proof (see the prior subsection for details). Hence we need to define a slightly different reduction. We alter the reduction in two ways: 
\begin{enumerate}
