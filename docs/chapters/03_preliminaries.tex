...
The \textit{do \{\dots\}} block captures the monad. Using $\leftarrow$, which stands for the monadic binding operation, a (uniformly random) elementary event (i.e. an element of S) is bound to x. The result is a dspd, the spmf f applied to every element from S.
