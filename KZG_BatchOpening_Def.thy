...
text \<open>We define EvalBatch according to CreateWitnessBatch in [KZG10] section 3.4 Batch Opening.
The function reveals all points (i,\<phi>(i)) for i \<in> B using only one witness value. 
It returns (B,r(x),w_i), where B is the provided set of positions, r(x) is a polynomial holding all 
evaluations (i.e. r(i)=\<phi>(i) for all i \<in> B), and w_B is the witness for B and r(x).\<close>
...
where 
  "eval_batch PK td \<phi> B =( 
    let r = r B \<phi>; \<comment>\<open>remainder of \<phi>(x)/(\<Prod>i\<in>B. (x-i)) i.e. \<phi>(x) mod (\<Prod>i\<in>B. (x-i))\<close>
        ...
        w_B = g_pow_PK_Prod PK \<psi> \<comment>\<open>g^\<psi>(\<alpha>)\<close>
    in
    (r, w_B) \<comment>\<open>(B, r(x), g^\<psi>(\<alpha>))\<close>
  )" 
...
definition valid_poly_batch::"'e mod_ring poly \<Rightarrow> bool"
  where "valid_poly_batch \<phi> = (degree \<phi> \<le> max_deg)"
...
sublocale BatchEvalKZG: abstract_polynomial_commitment_scheme key_gen commit verify_poly eval_batch 
  verify_eval_batch valid_poly_batch valid_argument_batch valid_eval_batch .
