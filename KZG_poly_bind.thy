...
imports KZG_correct CryptHOL_ext "Sigma_Commit_Crypto.Commitment_Schemes" "tSDH_assumption"
...
subsection \<open>t-SDH game\<close>
text \<open>We reduce the polynomial binding to the t-SDH assumption\<close>
...
\<close>
fun bind_reduction
  :: "('a ck, 'e mod_ring poly, 'a commit, unit) bind_adversary \<Rightarrow> ('a,'e) t_SDH.adversary"                     
where
  "bind_reduction \<A> PK = do {
  (C, \<phi>, _, \<phi>', _) \<leftarrow> \<A> PK;
  let \<alpha> = find_\<alpha> (PK!1) (\<phi> - \<phi>');
...
allowing us to prove the equivalence of binding game to t-SDH game with stronger_bind_reduction. 
From this equivalence it is easy to proof the theorem winnnng the binding game is less than or equal 
to breaking the t-SDH assumption\<close>
fun stronger_bind_reduction
  :: "('a ck, 'e mod_ring poly, 'a commit, unit)  bind_adversary \<Rightarrow> ('a,'e) t_SDH.adversary"                     
where
  "stronger_bind_reduction \<A> PK = do {
...
theorem poly_bind_game_eq_t_SDH_strong_red: 
  shows "cs.bind_game \<A> = t_SDH_G\<^sub>p.game (stronger_bind_reduction \<A>)"
proof -
  note [simp] = Let_def split_def
...
lemma t_SDH_advantage_stronger_red_le_red: 
"t_SDH_G\<^sub>p.advantage (stronger_bind_reduction \<A>) \<le> t_SDH_G\<^sub>p.advantage (bind_reduction \<A>)"
  apply (simp only: t_SDH_G\<^sub>p.advantage_def t_SDH_G\<^sub>p.game_def 
...
theorem polynomial_binding: 
  "cs.bind_advantage \<A>  \<le>  t_SDH_G\<^sub>p.advantage (bind_reduction \<A>)"
  apply (simp add: cs.bind_advantage_def)
  apply (simp add: poly_bind_game_eq_t_SDH_strong_red)
  apply (insert t_SDH_advantage_stronger_red_le_red)
  apply (simp add: t_SDH_G\<^sub>p.advantage_def)
  done
